...
func _ready():
	Dialogic.start("missionBriefing3")
...
func turnScreenOn(argument: String):
	if argument == "screen_on":
		var tween = get_tree().create_tween().set_loops().bind_node($Control/NinePatchRect/Control)
...
func transitionLevel(argument: String):
	if argument == "transition":
		...
		get_tree().change_scene_to_file("res://Scene/Story Scenes/credit.tscn")
